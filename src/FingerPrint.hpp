...
#pragma omp atomic update
				...
				// m_counts.modify_if(*itr, [multiplier](std::pair<const uint64_t, size_t> &item)
...
#pragma omp atomic update
	...
	void printCountsMode() const
	{
		for (unsigned y = 0; y < opt::snp.size(); ++y)
		{
			...
			{
				std::cerr << "file " << opt::snp[y] << " can not be opened" << std::endl;
				exit(1);
			}
			std::ofstream file(opt::name + "_" + name1 + ".txt");

			if (file.is_open())
			{
				std::cerr << "file " << opt::name + "_" + name1 + ".txt" << " is opened" << std::endl;
				...
				for (size_t i = 0; i < m_alleleIDs[y].size(); ++i)
				{
					...
					auto calculateModeAdjusted = [&](const vector<uint64_t> &allele, unsigned &countSum) -> int
					{
						...
						for (auto id : allele)
						{
							...
							{
								if (freq >= mode - 2 && freq <= mode + 2) // 最高频数的数字±2，的个数之和
								{
									++countInRange;
									sumInRange += freq;
								}
...
private:
	...
	// 										   std::allocator<std::pair<const uint64_t, size_t>>,
	// 										   6,
	// 										   std::mutex>;
	// // Map1 m_counts;
	// tsl::robin_map<uint64_t, size_t, robin_hood::hash<uint64_t>> m_counts; // k-mer to count
...
#pragma omp critical(stderr)
	...
	// 			}
	// 		}
	// 		cerr << "m_alleleIDs " << m_alleleIDs.size() << endl;
	// 		cerr << "m_alleleIDToKmerRef " << m_alleleIDToKmerRef.size() << endl;
	// 		cerr << "m_alleleIDToKmerVar " << m_alleleIDToKmerVar.size() << endl;
...
#pragma omp critical(stderr)
			...
			while (l >= 0)
			{
				...
				{
					unsigned index = entryNum / 2;
					m_alleleIDToKmerRef[i].emplace_back(shared_ptr<vector<uint64_t>>(new vector<uint64_t>()));
					// k-merize and
					for (KseqHashIterator itr(seq->seq.s, seq->seq.l, opt::k);
						 itr != itr.end(); ++itr)
					{
				...
				{
					unsigned index = entryNum / 2;
					m_alleleIDToKmerVar[i].emplace_back(shared_ptr<vector<uint64_t>>(new vector<uint64_t>()));
					// k-merize and insert
					for (KseqHashIterator itr(seq->seq.s, seq->seq.l, opt::k);
						 itr != itr.end(); ++itr)
					{
